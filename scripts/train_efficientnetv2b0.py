"""Training and screening script for EfficientNetV2B0 Expérience A.

This script implements the fair comparison training pipeline for EfficientNetV2B0,
including fold-specific class weight verification against archived DenseNet121 D,
dataset preprocessing validation, exact callback configuration, OOF prediction
export, and screening summary generation.

IMPORTANT: Per project rules, do not launch scientific training automatically.
"""
...
def verify_fold_class_weights_against_archived(config: dict[str, Any], fold: int) -> dict[int, float]:
    """Load and verify class weights against archived DenseNet121 Exp D fold weights.
    ...
    Raises:
        ValueError: If computed weights differ from archived weights.
    """
    computed_weights, _ = _compute_fold_class_weights(config, fold)

    archived_path = Path("artifacts/models/densenet121_exp_d_v1/evaluation") / f"fold_{fold}_class_weights.json"
    if archived_path.is_file():
        with open(archived_path, "r", encoding="utf-8") as f:
            archived_data = json.load(f)

        archived_cw_dict = archived_data.get("class_weights", {})
        archived_weights: dict[int, float] = {}
        for class_info in archived_cw_dict.values():
            cid = int(class_info["class_id"])
            w = float(class_info["weight"])
            archived_weights[cid] = w

        for cid, comp_w in computed_weights.items():
            arch_w = archived_weights.get(cid)
            if arch_w is None:
                raise ValueError(f"Class id {cid} missing from archived class weights for fold {fold}")
            if abs(comp_w - arch_w) > 1e-4:
                raise ValueError(
                    f"Class weight mismatch for fold {fold}, class_id {cid}: "
                    f"computed={comp_w:.6f}, archived={arch_w:.6f}. "
                    "Fair comparison requires identical class weights."
                )
        logger.info(f"Verified class weights for fold {fold} match archived DenseNet121 D exactly.")
    else:
        logger.warning(
            f"Archived class weights not found at {archived_path}. Using newly computed balanced weights."
        )
...
def generate_screening_summary(output_dir: Path, screening_folds: list[int] = [0, 3, 4]) -> bool:
    ...
    """
    metrics_dir = output_dir / "reports" / "efficientnetv2b0" / "metrics"
    pred_dir = output_dir / "reports" / "efficientnetv2b0" / "predictions"
    fold_metrics = {}
    oof_images_list = []
    ...
    summary = {
        ...
        "experiment": "Exp A (Fair Comparison)",
        ...
        "conclusion": (
            ...
            "Do not automatically declare EfficientNet superior."
        ),
    }

    summary_path = output_dir / "reports" / "efficientnetv2b0" / "efficientnetv2b0_screening_summary.json"
...
def train_fold_efficientnet(config: dict[str, Any], fold: int, output_dir: Path) -> dict[str, Any]:
    """Train EfficientNetV2B0 on a single fold.
    ...
    logger.info(f"\n==========================================")
    logger.info(f"Starting EfficientNetV2B0 Fold {fold} (seed={seed})")
    logger.info(f"==========================================")

    # 1. Verify class weights
    class_weights = verify_fold_class_weights_against_archived(config, fold)
    ...
    validate_efficientnet_strategy(model, config)

    fold_ckpt_dir = output_dir / "models" / "efficientnetv2b0" / "checkpoints" / f"fold_{fold}"
    ...
    # 8. Export OOF Predictions
    pred_dir = output_dir / "reports" / "efficientnetv2b0" / "predictions"
    export_oof_predictions(model, val_ds, config, fold, pred_dir / f"fold_{fold}_oof_predictions.csv")

    # 9. Save History (JSON and CSV)
    hist_dir = output_dir / "reports" / "efficientnetv2b0" / "history"
...
def main() -> None:
    """CLI entrypoint for training EfficientNetV2B0."""
    parser = argparse.ArgumentParser(description="Train EfficientNetV2B0 Exp A")
    ...
    for fold in args.folds:
        train_fold_efficientnet(config, fold, output_dir)
